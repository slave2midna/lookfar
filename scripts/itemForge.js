  ...
  function openItemForgeDialog() {
    ...
    const dlg = new Dialog({
      ...
      render: async (html) => {
        ...
        // Backing arrays for current render state
        ...
        // ---------- Shared selectable list behavior (index-based) ----------
        const wireSelectableList = ($container, itemSel, { onSelect } = {}) => {
          ...
          // auto-select first item if any (fires onSelect)
        ...
        const renderTemplates = (rows) => {
          ...
          wireSelectableList($templateList, ".if-template", {
            onSelect: (el) => updateHandToggle(el)
          });
        };
        ...
        // Now index-based selection → use the backing list instead of name matching
        ...
        const updateForKind = (kind) => {
          ...
          if (kind !== "weapon") updateHandToggle(); // keeps behavior for non-weapons
          relayout();
        };
