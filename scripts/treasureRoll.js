...
// Currency Generation
function rollCurrency(remainingBudget, maxVal, {
    minAmount = 1,
    roundTo = 1
} = {}) {
...
async function rollCustom() {
    ...
    if (!doc && result.pack && result.documentId) {
        ...
        if (pack) {
            try {
                doc = await pack.getDocument(result.documentId);
            } catch {}
        }
        if (!doc) {
            const compUuid = `Compendium.${result.pack}.${result.documentId}`;
            try {
                doc = await fromUuid(compUuid);
            } catch {}
        }
    }
...
async function createStash(items, cacheFolder, currencyTotal = 0) {
    ...
    const stashName = game.i18n.format("LOOKFAR.TreasureRoll.Stash.DefaultName", {
        num: nextNum
    });
...
async function renderTreasureResultDialog(items, budget, config) {
    ...
    const tempItems = await Promise.all(docInputs.map(async (data) => {
        ...
        if ("taste" in data) {
            ...
            itemData = {
                ...
                system: {
                    category: {
                        value: baseWeapon?.category || ""
                    },
                    hands: {
                        value: baseWeapon?.hand || ""
                    },
                    type: {
                        value: baseWeapon?.type || ""
                    },
                    attributes: {
                        primary: {
                            value: baseWeapon?.attrA || ""
                        },
                        secondary: {
                            value: baseWeapon?.attrB || ""
                        }
                    },
                    accuracy: {
            ...
            itemData = {
                ...
                system: {
                    def: {
                        value: baseAccessory?.def ?? 0
                    },
                    mdef: {
                        value: baseAccessory?.mdef ?? 0
                    },
                    init: {
    ...
    const enrichedHtml = await TextEditor.enrichHTML(innerHtml, {
        async: true
    });
    ...
    Hooks.on("lookfarShowTreasureRollDialog", (rerollConfig = null) => {
        (async () => {
            ...
            const {
                weaponList,
                weaponQualities
            } = dataLoader.weaponsData;
            const {
                armorList,
                armorQualities
            } = dataLoader.armorData;
            const {
                shieldList,
                shieldQualities
            } = dataLoader.shieldsData;
            const {
                accessoryList,
                accessoryQualities
            } = dataLoader.accessoriesData;
                ...
                while ((ignoreValues || remainingBudget > 0) && items.length < itemCount && failedAttempts < maxAttempts) {
                    ...
                    switch (type) {
                        ...
                        case "Ingredient":
                            ...
                            break;
                        case "Currency":
                            item = rollCurrency(remainingBudget, maxVal, {
                                roundTo: 10
                            });
                            break;
                        case "Custom":
